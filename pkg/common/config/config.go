...
import (
	...
	"github.com/openimsdk/open-im-server/v3/tools/db/redisutil"
...
type Group struct {
	...
	GroupLimits                struct {
		MaxMembersPerGroup     int  `yaml:"maxMembersPerGroup"`
		EnableMemberLimitCheck bool `yaml:"enableMemberLimitCheck"`
	} `yaml:"groupLimits"`
}
...
type Aws struct {
	...
	SessionToken    string `yaml:"sessionToken"`
	PublicRead      bool   `yaml:"publicRead"`
	URLStyle        string `yaml:"urlStyle"` // "virtual-hosted" 或 "path-style", 默认为 "virtual-hosted"
}
