...
func UserPb2DBMap(user *sdkws.UserInfo) map[string]any {
	...
	fields := map[string]any{
		...
		"can_send_free_msg":   user.CanSendFreeMsg,
		"org_id":              user.OrgId,
	...
	for key, value := range fields {
		if v, ok := value.(string); ok && v != "" {
			...
			// 对于 canSendFreeMsg 和 globalRecvMsgOpt 字段，允许设置为 0
			if key == "can_send_free_msg" || v != 0 {
				val[key] = v
			}
		}
	}
...
func UserPb2DBMapEx(user *sdkws.UserInfoWithEx) map[string]any {
	...
	if user.CanSendFreeMsg != nil {
		val["can_send_free_msg"] = user.CanSendFreeMsg.Value
	}
