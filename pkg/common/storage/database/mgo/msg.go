...
import (
	...
	// Import for encryption utilities
	"github.com/openimsdk/open-im-server/v3/pkg/util/crypto"
	"github.com/openimsdk/tools/log" // Assuming tools/log for logging
)
...
// encryptMsgDataModel 对 MsgDataModel 的 Content 字段进行加密（如果不为空）。
func (m *MsgMgo) encryptMsgDataModel(ctx context.Context, msgData *model.MsgDataModel) {
	if msgData == nil || msgData.Content == "" {
		return
	}

	// 获取AES密钥
	key := crypto.GetFixedKey()

	// 如果密钥为空，不进行加密
	if key == nil || len(key) != crypto.AESKeySize {
		return
	}

	encryptedContent, err := crypto.Encrypt([]byte(msgData.Content), key)
	if err != nil {
		log.ZError(ctx, "消息加密失败", err, "clientMsgID", msgData.ClientMsgID)
		return
	}
	msgData.EncryptedContent = encryptedContent
	msgData.EncryptionType = crypto.EncryptionTypeAES256GCM
	msgData.Content = ""
}

// decryptMsgDataModel 解密 MsgDataModel 的 EncryptedContent 字段（如果已加密）。
func (m *MsgMgo) decryptMsgDataModel(ctx context.Context, msgData *model.MsgDataModel) {
	if msgData == nil || msgData.EncryptedContent == "" || msgData.EncryptionType != crypto.EncryptionTypeAES256GCM {
		return
	}
	...
	// 如果密钥为空，无法解密
	if key == nil || len(key) != crypto.AESKeySize {
		log.ZWarn(ctx, "消息解密失败: 密钥未配置", nil, "clientMsgID", msgData.ClientMsgID)
		msgData.Content = msgData.EncryptedContent // 直接使用加密内容
		return
	}

	decryptedContent, err := crypto.Decrypt(msgData.EncryptedContent, key)
	if err != nil {
		log.ZError(ctx, "消息解密失败", err, "clientMsgID", msgData.ClientMsgID)
		msgData.Content = "[解密错误]"
		return
	}
	msgData.Content = string(decryptedContent)
}

func (m *MsgMgo) Create(ctx context.Context, msg *model.MsgDocModel) error {
	if msg != nil && len(msg.Msg) > 0 {
		for _, msgInfo := range msg.Msg {
			if msgInfo != nil && msgInfo.Msg != nil {
				m.encryptMsgDataModel(ctx, msgInfo.Msg)
			}
		}
	}
...
func (m *MsgMgo) UpdateMsg(ctx context.Context, docID string, index int64, key string, value any) (*mongo.UpdateResult, error) {
	...
	// 处理当key为"msg"时的场景（来自BatchInsertBlock的直接调用）
	if key == "msg" {
		// 尝试直接转换为MsgDataModel（BatchInsertBlock调用时是这种类型）
		msgData, ok := value.(*model.MsgDataModel)
		if ok && msgData != nil && msgData.Content != "" {
			// 如果有明文内容，则进行加密
			m.encryptMsgDataModel(ctx, msgData)
			// msgData引用已被修改，无需额外操作
		} else {
			// 处理可能的map类型情况（其他调用场景）
			...
			if ok {
				if content, exists := msgMap["content"]; exists {
					if contentStr, ok := content.(string); ok && contentStr != "" {
						// 为map类型创建临时对象进行加密
						tempMsg := &model.MsgDataModel{Content: contentStr}
						m.encryptMsgDataModel(ctx, tempMsg)

						// 更新map中的值
						msgMap["encrypted_content"] = tempMsg.EncryptedContent
						msgMap["encryption_type"] = tempMsg.EncryptionType
						msgMap["content"] = ""
					}
				}
			}
...
func (m *MsgMgo) FindOneByDocID(ctx context.Context, docID string) (*model.MsgDocModel, error) {
	msgDoc, err := mongoutil.FindOne[*model.MsgDocModel](ctx, m.coll, bson.M{"doc_id": docID})
	if err != nil {
		return nil, err
	}
	// Decrypt content for each message in the document after retrieval
	if msgDoc != nil && len(msgDoc.Msg) > 0 {
		for _, msgInfo := range msgDoc.Msg {
			if msgInfo != nil && msgInfo.Msg != nil {
				m.decryptMsgDataModel(ctx, msgInfo.Msg)
			}
		}
	}
	return msgDoc, nil
...
func (m *MsgMgo) getMsgBySeqIndexIn1Doc(ctx context.Context, userID, docID string, seqs []int64) ([]*model.MsgInfoModel, error) {
	...
	for i := range msgDocModel[0].Msg {
		...
		if msg.Msg != nil {
			m.decryptMsgDataModel(ctx, msg.Msg)
		}
	...
	for skip := int64(0); ; skip++ {
		...
		for i := len(msgDocModel.Msg) - 1; i >= 0; i-- {
			if msgDocModel.Msg[i].Msg != nil {
				m.decryptMsgDataModel(ctx, msgDocModel.Msg[i].Msg)
...
func (m *MsgMgo) GetMsgDocModelByIndex(ctx context.Context, conversationID string, index, sort int64) (*model.MsgDocModel, error) {
	...
	if len(msgs) > 0 {
		// 对文档中的所有消息进行解密处理
		for _, msgInfo := range msgs[0].Msg {
			if msgInfo != nil && msgInfo.Msg != nil {
				m.decryptMsgDataModel(ctx, msgInfo.Msg)
			}
		}
...
func (m *MsgMgo) SearchMessage(ctx context.Context, req *msg.SearchMessageReq) (int64, []*model.MsgInfoModel, error) {
	...
	for _, val := range data {
		...
		for _, i := range val.Index {
			...
			// 解密搜索结果中的消息内容
			if res.Msg[i] != nil && res.Msg[i].Msg != nil {
				m.decryptMsgDataModel(ctx, res.Msg[i].Msg)
			}
...
func (m *MsgMgo) GetRandBeforeMsg(ctx context.Context, ts int64, limit int) ([]*model.MsgDocModel, error) {
	msgDocs, err := mongoutil.Aggregate[*model.MsgDocModel](ctx, m.coll, []bson.M{
		{
			"$match": bson.M{
				"msgs": bson.M{
					"$not": bson.M{
...
func (m *MsgMgo) GetLastMessage(ctx context.Context, conversationID string) (*model.MsgInfoModel, error) {
	...
	// 解密最后一条消息的内容
	if res[0].Msgs != nil && res[0].Msgs.Msg != nil {
		m.decryptMsgDataModel(ctx, res[0].Msgs.Msg)
	}
...
func (m *MsgMgo) onlyFindDocIndex(ctx context.Context, docID string, indexes []int64) ([]*model.MsgInfoModel, error) {
	...
	// 解密所有消息内容
	for _, msgInfo := range msgDocModel[0].Msg {
		if msgInfo != nil && msgInfo.Msg != nil {
			m.decryptMsgDataModel(ctx, msgInfo.Msg)
		}
	}
