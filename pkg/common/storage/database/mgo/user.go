...
import (
	...
	"github.com/openimsdk/open-im-server/v3/protocol/user"
...
func (u *UserMgo) Find(ctx context.Context, userIDs []string) (users []*model.User, err error) {
	return mongoutil.Find[*model.User](ctx, u.coll, bson.M{"user_id": bson.M{"$in": userIDs}}, options.Find().SetProjection(bson.M{"aes_key": 0}))
}

func (u *UserMgo) Take(ctx context.Context, userID string) (user *model.User, err error) {
	return mongoutil.FindOne[*model.User](ctx, u.coll, bson.M{"user_id": userID}, options.FindOne().SetProjection(bson.M{"aes_key": 0}))
}

func (u *UserMgo) TakeNotification(ctx context.Context, level int64) (user []*model.User, err error) {
	return mongoutil.Find[*model.User](ctx, u.coll, bson.M{"app_manger_level": level}, options.Find().SetProjection(bson.M{"aes_key": 0}))
}

func (u *UserMgo) TakeGTEAppManagerLevel(ctx context.Context, level int64) (user []*model.User, err error) {
	return mongoutil.Find[*model.User](ctx, u.coll, bson.M{"app_manger_level": bson.M{"$gte": level}}, options.Find().SetProjection(bson.M{"aes_key": 0}))
}

func (u *UserMgo) TakeByNickname(ctx context.Context, nickname string) (user []*model.User, err error) {
	return mongoutil.Find[*model.User](ctx, u.coll, bson.M{"nickname": nickname}, options.Find().SetProjection(bson.M{"aes_key": 0}))
}

func (u *UserMgo) Page(ctx context.Context, pagination pagination.Pagination) (count int64, users []*model.User, err error) {
	return mongoutil.FindPage[*model.User](ctx, u.coll, bson.M{}, pagination, options.Find().SetProjection(bson.M{"aes_key": 0}))
}
...
func (u *UserMgo) PageFindUser(ctx context.Context, level1 int64, level2 int64, pagination pagination.Pagination) (count int64, users []*model.User, err error) {
	...
	return mongoutil.FindPage[*model.User](ctx, u.coll, query, pagination, options.Find().SetProjection(bson.M{"aes_key": 0}))
}
...
// GetUserAESKey 单独查询用户的AES密钥，确保敏感信息不会随其他请求泄露
func (u *UserMgo) GetUserAESKey(ctx context.Context, userID string) (aesKey string, err error) {
	// 只查询AES密钥字段
	projection := bson.M{"_id": 0, "aes_key": 1}
	result, err := mongoutil.FindOne[bson.M](ctx, u.coll, bson.M{"user_id": userID}, options.FindOne().SetProjection(projection))
	if err != nil {
		return "", errs.Wrap(err)
	}

	// 检查AES密钥是否存在
	if key, ok := result["aes_key"]; ok {
	...
	// 更新用户记录
	updateErr := mongoutil.UpdateOne(ctx, u.coll, bson.M{"user_id": userID}, bson.M{"$set": bson.M{"aes_key": newKey}}, true)
	if updateErr != nil {
...
// generateAESKey 生成一个新的AES-256密钥并返回Base64编码的字符串
