...
func (u *UserCacheRedis) GetUserAESKey(ctx context.Context, userID string) (aesKey string, err error) {
	return getCache(
		ctx,
		u.rcClient,
		u.getUserAESKeyKey(userID),
		u.expireTime,
		func(ctx context.Context) (string, error) {
			return u.userDB.GetUserAESKey(ctx, userID)
		},
	)
}

func (u *UserCacheRedis) DelUsersAESKey(userIDs ...string) cache.UserCache {
	keys := make([]string, 0, len(userIDs))
	for _, userID := range userIDs {
		keys = append(keys, u.getUserAESKeyKey(userID))
	}
	cache := u.CloneUserCache()
	cache.AddKeys(keys...)
