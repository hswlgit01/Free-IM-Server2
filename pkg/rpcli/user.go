...
func (x *UserClient) GetUserAESKey(ctx context.Context, userID string) (string, error) {
	if userID == "" {
		return "", nil
	}

	// 直接调用RPC方法获取AES密钥
	resp, err := x.UserClient.GetUserAESKey(ctx, &user.GetUserAESKeyReq{UserID: userID})
	if err != nil {
		return "", err
	}

	return resp.AesKey, nil
}
