...
func (c *Client) ResetClient(ctx *UserConnContext, conn LongConn, longConnServer LongConnServer, userClient *rpcli.UserClient) {
	c.w = new(sync.Mutex)
	c.conn = conn
	...
	if c.SDKType == GoSDK {
		c.Encoder = NewAESGobEncoder(userClient, c)
	} else {
