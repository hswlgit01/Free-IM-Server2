...
func (u *UserApi) GetUserKeyByRSA(c *gin.Context) {
	userID, exists := c.Get(constant.OpUserID)
	if !exists {
		apiresp.GinError(c, errs.New("not found user"))
		return
	}
	userIDStr := userID.(string)
	resp, err := u.Client.GetUserAESKey(c, &user.GetUserAESKeyReq{UserID: userIDStr})
	if err != nil {
	...
	encrypt, err := RSAEncrypt([]byte(key), req.RSAPublicKey)
	if err != nil {
		apiresp.GinError(c, err)
		return
	}
	apiresp.GinSuccess(c, encrypt)
}
