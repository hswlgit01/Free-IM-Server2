...
import (
	...
	"github.com/openimsdk/open-im-server/v3/protocol/conversation"
...
func newGinRouter(ctx context.Context, client discovery.Conn, cfg *Config) (*gin.Engine, error) {
	...
	u := NewUserApi(user.NewUserClient(userConn), client, cfg.Discovery.RpcService)
	{
		...
		userRouterGroup.POST("/get_user_key", u.GetUserKeyByRSA)
	...
	g := NewGroupApi(group.NewGroupClient(groupConn))
	{
		...
		groupRouterGroup.POST("/create_group_operation_log", g.CreateGroupOperationLog)
	}
	// certificate
	{
		...
		authRouterGroup.POST("/batch_force_logout", a.BatchForceLogout)
	}
	// encryption management
	{
		//c := NewCryptoApi(cfg.Share.IMAdminUserID)
		//cryptoRouterGroup := r.Group("/crypto")
		//cryptoRouterGroup.POST("/generate_key_pair", c.GenerateKeyPair)
		//cryptoRouterGroup.POST("/set_aes_key", c.SetAESKey)
	}
	// Third service
	{
		t := NewThirdApi(third.NewThirdClient(thirdConn), cfg.API.Prometheus.GrafanaURL, &cfg.Third, &cfg.Minio)
		...
		objectGroup.PUT("/s3_proxy/*filepath", t.S3Proxy)
