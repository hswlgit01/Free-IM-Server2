...
func (m *msgServer) sendMarkAsReadNotification(ctx context.Context, conversationID string, sessionType int32, sendID, recvID string, seqs []int64, hasReadSeq int64) {
	// 群聊不发送已读回执，避免会话列表出现「某某:[新消息]」占位（普通用户在禁言群无法发消息，易造成混淆）
	if sessionType == constant.ReadGroupChatType {
		return
	}
	...
	if sessionType == constant.SingleChatType && sendID != recvID {
		log.ZDebug(ctx, "发送单聊已读回执", "readerUserID", sendID, "toSenderUserID", recvID, "conversationID", conversationID, "hasReadSeq", hasReadSeq)
	}
	// 使用专用的读回执发送方法
