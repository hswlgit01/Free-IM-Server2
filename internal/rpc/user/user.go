...
import (
	...
	friendpb "github.com/openimsdk/open-im-server/v3/protocol/relation"
	"github.com/openimsdk/open-im-server/v3/protocol/sdkws"
	pbuser "github.com/openimsdk/open-im-server/v3/protocol/user"
...
func (s *userServer) UserRegister(ctx context.Context, req *pbuser.UserRegisterReq) (resp *pbuser.UserRegisterResp, err error) {
	...
	for _, user := range req.Users {
		// 为每个用户生成AES密钥
		aesKey, keyErr := generateAESKey()
		if keyErr != nil {
			return nil, errs.Wrap(keyErr)
		}
		...
		users = append(users, &tablerelation.User{
			...
			CanSendFreeMsg:   user.CanSendFreeMsg,
			AESKey:           aesKey,
		})
	}
...
// GetUserAESKey 获取用户的AES加密密钥
func (s *userServer) GetUserAESKey(ctx context.Context, req *pbuser.GetUserAESKeyReq) (*pbuser.GetUserAESKeyResp, error) {
	// 检查请求参数
	if req.UserID == "" {
		return nil, errs.ErrArgs.WithDetail("用户ID不能为空")
	}

	// 先检查用户是否存在
	exist, err := s.db.IsExist(ctx, []string{req.UserID})
	if err != nil {
		return nil, err
	}
	if !exist {
		return nil, errs.ErrRecordNotFound.WithDetail("用户不存在")
	}

	// 获取用户的AES密钥
	aesKey, err := s.db.GetUserAESKey(ctx, req.UserID)
	if err != nil {
		return nil, err
	}

	return &pbuser.GetUserAESKeyResp{
		AesKey: aesKey,
	}, nil
}
