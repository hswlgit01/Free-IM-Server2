...
type groupServer struct {
	...
	groupOpLogDB       controller.GroupOperationLogController
...
func Start(ctx context.Context, config *Config, client discovery.Conn, server grpc.ServiceRegistrar) error {
	...
	groupOpLogDB, err := mgo.NewGroupOperationLogMongo(mgocli.GetDB())
	if err != nil {
		return err
	}
...
func (g *groupServer) CreateGroup(ctx context.Context, req *pbgroup.CreateGroupReq) (*pbgroup.CreateGroupResp, error) {
	//if err := authverify.CheckAdmin(ctx, g.config.Share.IMAdminUserID); err != nil {
	//	return nil, err
	//}
	...
	// 检查群组初始成员数是否超过限制
	if g.config.RpcConfig.GroupLimits.EnableMemberLimitCheck && len(userIDs) > g.config.RpcConfig.GroupLimits.MaxMembersPerGroup {
		return nil, errs.ErrArgs.WrapMsg(fmt.Sprintf("group members exceed maximum limit of %d", g.config.RpcConfig.GroupLimits.MaxMembersPerGroup))
	}

	info, err := g.userClient.GetUserInfo(ctx, req.OwnerUserID)
	if err != nil {
		return nil, err
	}
	//// 判断是否是超管，组织超管，组织管理员，组织后台管理员
	//if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) && info.OrgRole == "Normal" {
	...
	req.GroupInfo.OrgID = info.OrgId
	...
	// 记录创建群组操作日志
	createDetails := &controller.CreateGroupDetails{
	...
	for _, member := range groupMembers {
		orgId := userMap[member.UserID].OrgId
		if orgId != info.OrgId {
			return nil, servererrs.ErrDifferentOrg.WrapMsg("cannot add member from different organization")
		}
	...
	// 记录创建群组操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		if err := g.groupOpLogDB.RecordCreateGroupOperation(ctx, group.GroupID, opUserID, createDetails); err != nil {
			log.ZWarn(ctx, "记录创建群组操作日志失败", err, "groupID", group.GroupID, "operator", opUserID)
		}
	}
	return resp, nil
}
...
func (g *groupServer) InviteUserToGroup(ctx context.Context, req *pbgroup.InviteUserToGroupReq) (*pbgroup.InviteUserToGroupResp, error) {
	...
	// 检查邀请后是否会超过群组成员数限制
	currentMemberCount, err := g.db.FindGroupMemberNum(ctx, req.GroupID)
	if err != nil {
		return nil, err
	}
	if g.config.RpcConfig.GroupLimits.EnableMemberLimitCheck && currentMemberCount+uint32(len(req.InvitedUserIDs)) > uint32(g.config.RpcConfig.GroupLimits.MaxMembersPerGroup) {
		return nil, errs.ErrArgs.WrapMsg(fmt.Sprintf("inviting %d members would exceed maximum group limit of %d (current: %d)", len(req.InvitedUserIDs), g.config.RpcConfig.GroupLimits.MaxMembersPerGroup, currentMemberCount))
	}
	...
	for _, userID := range req.InvitedUserIDs {
		if userMap[userID].OrgId != group.OrgID {
			return nil, servererrs.ErrDifferentOrg.WrapMsg("cannot invite user to group from different organization")
		}
	}
	...
	// 检查群组邀请策略
	switch group.NeedVerification {
	case constant.GroupJoinDisabled:
		// 群组禁止加入，只有群主和管理员可以邀请
		if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
			if !(groupMember.RoleLevel == constant.GroupOwner || groupMember.RoleLevel == constant.GroupAdmin) {
				return nil, servererrs.ErrGroupJoinDisabled.WrapMsg("group joining is disabled, only group owner and admins can invite members")
			}
		}
	case constant.AllNeedVerification:
		if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
			if !(groupMember.RoleLevel == constant.GroupOwner || groupMember.RoleLevel == constant.GroupAdmin) {
...
func (g *groupServer) GetGroupMemberList(ctx context.Context, req *pbgroup.GetGroupMemberListReq) (*pbgroup.GetGroupMemberListResp, error) {
	...
	if req.Keyword == "" {
		...
		// dawn 2026-06-14 优化3万人群成员搜索：关键字查询直接数据库分页，避免先取全群成员再内存过滤。
		total, members, err = g.db.SearchGroupMember(ctx, req.Keyword, req.GroupID, req.Pagination)
	}
	if err != nil {
		return nil, err
	}
...
func (g *groupServer) KickGroupMember(ctx context.Context, req *pbgroup.KickGroupMemberReq) (*pbgroup.KickGroupMemberResp, error) {
	...
	// 批量记录踢人操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		for _, userID := range req.KickedUserIDs {
			if err := g.groupOpLogDB.RecordKickMemberOperation(ctx, req.GroupID, opUserID, userID, req.Reason); err != nil {
				log.ZWarn(ctx, "记录踢人操作日志失败", err, "groupID", req.GroupID, "operator", opUserID, "target", userID)
			}
		}
	}
...
func (g *groupServer) GetGroupsInfo(ctx context.Context, req *pbgroup.GetGroupsInfoReq) (*pbgroup.GetGroupsInfoResp, error) {
	...
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		opUserID := mcontext.GetOpUserID(ctx)
		info, err := g.userClient.GetUserInfo(ctx, opUserID)
		if err != nil {
			return nil, err
		}
		// 过滤不同组织的群组
		filteredGroups := make([]*sdkws.GroupInfo, 0, len(groups))
		for _, group := range groups {
			if group.OrgID == info.OrgId {
...
func (g *groupServer) GroupApplicationResponse(ctx context.Context, req *pbgroup.GroupApplicationResponseReq) (*pbgroup.GroupApplicationResponseResp, error) {
	...
	if (!inGroup) && req.HandleResult == constant.GroupResponseAgree {
		// 检查同意加群申请是否会超过群组成员数限制
		currentMemberCount, err := g.db.FindGroupMemberNum(ctx, req.GroupID)
		if err != nil {
			return nil, err
		}
		if g.config.RpcConfig.GroupLimits.EnableMemberLimitCheck && currentMemberCount+1 > uint32(g.config.RpcConfig.GroupLimits.MaxMembersPerGroup) {
			return nil, errs.ErrArgs.WrapMsg(fmt.Sprintf("approving group application would exceed maximum group limit of %d (current: %d)", g.config.RpcConfig.GroupLimits.MaxMembersPerGroup, currentMemberCount))
		}
...
func (g *groupServer) JoinGroup(ctx context.Context, req *pbgroup.JoinGroupReq) (*pbgroup.JoinGroupResp, error) {
	...
	if user.OrgId != group.OrgID {
		return nil, servererrs.ErrDifferentOrg.WrapMsg("cannot join group from different organization")
	}
	...
	case constant.Directly:
		...
		if g.config.RpcConfig.GroupLimits.EnableMemberLimitCheck && currentMemberCount+1 > uint32(g.config.RpcConfig.GroupLimits.MaxMembersPerGroup) {
			return nil, errs.ErrArgs.WrapMsg(fmt.Sprintf("joining group would exceed maximum group limit of %d (current: %d)", g.config.RpcConfig.GroupLimits.MaxMembersPerGroup, currentMemberCount))
		}
...
func (g *groupServer) TransferGroupOwner(ctx context.Context, req *pbgroup.TransferGroupOwnerReq) (*pbgroup.TransferGroupOwnerResp, error) {
	...
	// 记录转移群主操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		if err := g.groupOpLogDB.RecordTransferOwnerOperation(ctx, req.GroupID, mcontext.GetOpUserID(ctx), req.NewOwnerUserID, oldOwner.UserID); err != nil {
			log.ZWarn(ctx, "记录转移群主操作日志失败", err, "groupID", req.GroupID, "operator", mcontext.GetOpUserID(ctx), "newOwner", req.NewOwnerUserID)
		}
	}
...
func (g *groupServer) DismissGroup(ctx context.Context, req *pbgroup.DismissGroupReq) (*pbgroup.DismissGroupResp, error) {
	...
	// 记录解散群组操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		if err := g.groupOpLogDB.RecordDismissGroupOperation(ctx, req.GroupID, mcontext.GetOpUserID(ctx)); err != nil {
			log.ZWarn(ctx, "记录解散群组操作日志失败", err, "groupID", req.GroupID, "operator", mcontext.GetOpUserID(ctx))
		}
	}
...
func (g *groupServer) MuteGroupMember(ctx context.Context, req *pbgroup.MuteGroupMemberReq) (*pbgroup.MuteGroupMemberResp, error) {
	...
	// 记录禁言群成员操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		muteEndTime := time.Now().Add(time.Second * time.Duration(req.MutedSeconds))
		if err := g.groupOpLogDB.RecordMuteMemberOperation(ctx, req.GroupID, mcontext.GetOpUserID(ctx), req.UserID, int64(req.MutedSeconds), muteEndTime); err != nil {
			log.ZWarn(ctx, "记录禁言群成员操作日志失败", err, "groupID", req.GroupID, "operator", mcontext.GetOpUserID(ctx), "target", req.UserID)
		}
	}
...
func (g *groupServer) CancelMuteGroupMember(ctx context.Context, req *pbgroup.CancelMuteGroupMemberReq) (*pbgroup.CancelMuteGroupMemberResp, error) {
	...
	// 记录取消禁言群成员操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		if err := g.groupOpLogDB.RecordCancelMuteMemberOperation(ctx, req.GroupID, mcontext.GetOpUserID(ctx), req.UserID); err != nil {
			log.ZWarn(ctx, "记录取消禁言群成员操作日志失败", err, "groupID", req.GroupID, "operator", mcontext.GetOpUserID(ctx), "target", req.UserID)
		}
	}
...
func (g *groupServer) MuteGroup(ctx context.Context, req *pbgroup.MuteGroupReq) (*pbgroup.MuteGroupResp, error) {
	...
	// 记录群禁言操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		var muteEndTime time.Time
		if err := g.groupOpLogDB.RecordMuteGroupOperation(ctx, req.GroupID, mcontext.GetOpUserID(ctx), 0, muteEndTime); err != nil {
			log.ZWarn(ctx, "记录群禁言操作日志失败", err, "groupID", req.GroupID, "operator", mcontext.GetOpUserID(ctx))
		}
	}
...
func (g *groupServer) CancelMuteGroup(ctx context.Context, req *pbgroup.CancelMuteGroupReq) (*pbgroup.CancelMuteGroupResp, error) {
	...
	// 记录取消群禁言操作日志 (如果不是admin用户)
	if !authverify.IsAppManagerUid(ctx, g.config.Share.IMAdminUserID) {
		if err := g.groupOpLogDB.RecordCancelMuteGroupOperation(ctx, req.GroupID, mcontext.GetOpUserID(ctx)); err != nil {
			log.ZWarn(ctx, "记录取消群禁言操作日志失败", err, "groupID", req.GroupID, "operator", mcontext.GetOpUserID(ctx))
		}
	}
...
func (g *groupServer) CreateGroupOperationLog(ctx context.Context, req *pbgroup.CreateGroupOperationLogReq) (*pbgroup.CreateGroupOperationLogResp, error) {
	// 验证只有admin才有权限调用
	if err := authverify.CheckAdmin(ctx, g.config.Share.IMAdminUserID); err != nil {
		return nil, errs.ErrNoPermission.WrapMsg("只有admin才能调用此接口")
	}

	// 验证参数
	if req.GroupID == "" {
		return nil, errs.ErrArgs.WrapMsg("groupID不能为空")
	}
	if req.OperatorUserID == "" {
		return nil, errs.ErrArgs.WrapMsg("operatorUserID不能为空")
	}
	if req.OperationType <= 0 {
		return nil, errs.ErrArgs.WrapMsg("operationType无效")
	}
	...
	if err := g.groupOpLogDB.CreateGroupOperationLog(ctx, log); err != nil {
		return nil, err
	}
